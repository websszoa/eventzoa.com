...
export default function AdminMemberDialog({
  member,
  currentAdminId,
}: {
  member: AdminMemberDialogData;
      ...
      <DialogTrigger
        render={
          <Button
            type="button"
            variant="ghost"
            className="group grid h-auto w-full grid-cols-[auto_minmax(0,1fr)] gap-4 rounded-none border-0 px-5 py-5 text-left font-normal transition-colors hover:bg-blue-50/50 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:px-7"
          />
        }
      >
        <PageMemberAvatar
          ...
          name={name}
          alt={`${name} 프로필`}
          className="size-12 border border-blue-100"
          fallbackClassName="bg-blue-100 font-bold text-blue-700"
        />
        <span className="min-w-0">
          <span className="flex flex-wrap items-center gap-2">
            <span className="truncate font-cafe24 text-xl font-bold text-slate-950 group-hover:text-blue-700">
              {name}
            </span>
            {member.role === "admin" && (
          ...
          <span className="mt-1 block truncate text-xs text-slate-500">
            {member.email || "이메일 없음"} · {member.provider} · 방문{" "}
            {member.visit_count.toLocaleString("ko-KR")}회
          </span>
          <span className="mt-1 block text-xs text-slate-400">
            최근 방문 {formatDate(member.last_visited_at)}
          </span>
        </span>
        <ChevronRight
        ...
        <div className="bg-blue-50 px-6 py-7 sm:px-8">
          <div className="flex items-center gap-4 pr-10">
            <PageMemberAvatar
              ...
              name={name}
              alt={`${name} 프로필`}
              className="size-16 border-2 border-blue-100"
              fallbackClassName="bg-blue-100 font-cafe24 text-xl font-bold text-blue-700"
            />
            <DialogHeader className="min-w-0 gap-2">
              ...
              <DialogTitle className="truncate font-cafe24 text-3xl font-bold text-slate-950">
                {name}
              </DialogTitle>
              <DialogDescription className="truncate text-slate-500">
                {member.email || "이메일 없음"}
              </DialogDescription>
            </DialogHeader>
          </div>
        ...
        <div className="grid gap-6 px-6 pt-2 pb-7 sm:px-8 lg:grid-cols-[minmax(0,1fr)_220px]">
          <div className="divide-y divide-slate-200 rounded-2xl border border-slate-200">
            ...
            <InfoRow
              icon={CalendarDays}
              label="최근 방문"
              value={formatDate(member.last_visited_at, true)}
            />
            <InfoRow
              icon={CalendarDays}
              label="가입일"
          ...
          <div className="rounded-2xl bg-slate-50 p-5">
            <p className="text-sm text-blue-600">회원 설정</p>
            ...
            <PageAdminMemberSettings
              ...
              isCurrentAdmin={member.id === currentAdminId}
            />
          </div>
        </div>
      </DialogContent>
  ...
  return (
    <div className="px-4 py-3.5">
      <div className="flex items-center gap-2 text-xs text-slate-500">
        <Icon className="size-3.5 text-blue-700" aria-hidden="true" />
        {label}
      </div>
      <p className="mt-1.5 break-all text-sm text-slate-900">{value}</p>
    </div>
  );
}
