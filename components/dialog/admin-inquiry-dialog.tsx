    ...
    <Dialog>
      <DialogTrigger render={<Button type="button" variant="ghost" className="group grid h-auto w-full gap-3 rounded-none border-0 px-5 py-5 text-left font-normal transition-colors hover:bg-blue-50/50 sm:grid-cols-[120px_minmax(0,1fr)_auto] sm:items-center sm:px-7" />}>
        <span className="flex items-center gap-2 sm:block">
          ...
          <span className="mt-0 block text-xs text-slate-400 sm:mt-2">#{inquiry.id}</span>
        </span>
        <span className="min-w-0">
          <span className="block text-xs font-bold text-blue-600">{inquiryTypeLabels[inquiry.type]}</span>
          <span className="mt-1 block truncate font-cafe24 text-xl font-bold text-slate-950 group-hover:text-blue-700">{inquiry.subject}</span>
          <span className="mt-2 block truncate text-xs text-slate-500">{inquiry.name} · {inquiry.email} · {formatDate(inquiry.created_at)}</span>
        </span>
        ...
        <div className="bg-blue-50 px-6 py-7 sm:px-8">
          <DialogHeader className="gap-3 pr-10">
            <div className="flex flex-wrap items-center gap-2">
              <Badge className="rounded-full bg-blue-600 text-white">{inquiryTypeLabels[inquiry.type]}</Badge>
              <Badge variant="outline" className="rounded-full bg-white">{inquiryStatusLabels[inquiry.status]}</Badge>
              <span className="text-xs text-slate-400">#{inquiry.id}</span>
            </div>
            <DialogTitle className="font-cafe24 text-3xl leading-tight font-bold text-slate-950">{inquiry.subject}</DialogTitle>
            <DialogDescription className="text-slate-500">{inquiry.name}님의 문의입니다.</DialogDescription>
          </DialogHeader>
        </div>
        ...
        <div className="space-y-6 px-6 pt-2 pb-7 sm:px-8">
          <div className="min-w-0 space-y-5">
            <div className="grid gap-3 rounded-2xl bg-slate-50 p-4 sm:grid-cols-2">
              ...
              <InfoItem icon={CalendarDays} label="접수일" value={formatDate(inquiry.created_at)} />
              <InfoItem icon={CalendarDays} label="최근 변경" value={formatDate(inquiry.updated_at)} />
            </div>
            {relatedUrl && (
              <a href={relatedUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 break-all text-sm font-bold text-blue-700 hover:underline">
            ...
            <div>
              ...
              <div className="mt-2 whitespace-pre-wrap break-words rounded-2xl border border-slate-200 p-5 text-sm leading-7 text-slate-700">{inquiry.message}</div>
            </div>
          </div>
...
function InfoItem({ icon: Icon, label, value }: { icon: typeof UserRound; label: string; value: string }) {
  return (
    <div className="flex items-start gap-2.5">
      <Icon className="mt-0.5 size-4 shrink-0 text-blue-700" aria-hidden="true" />
