...
import { deleteEventByAdmin } from "@/lib/actions/action-event-delete";
import { updateEventByAdmin } from "@/lib/actions/action-event-edit";
import type { Event as EventItem, EventAddFormValues } from "@/lib/types";
import { eventAddSchema } from "@/lib/validations";
import { toDateTimeLocal, toIsoString } from "@/lib/utils";
...
function mapEventToFormValues(event: EventItem): EventAddFormValues {
  return {
    name: event.name ?? "",
    slug: event.slug ?? "",
    description: event.description ?? "",
    country: event.country ?? event.location?.country ?? "",
    region: event.region ?? event.location?.region ?? "",
    ...
    eventScale: event.event_scale != null ? String(event.event_scale) : "",
    eventSite: event.event_site ?? "",
    eventProgram: event.event_program ?? "",
    eventStartAt: toDateTimeLocal(event.event_start_at),
    eventEndAt: toDateTimeLocal(event.event_end_at),
    registrationStatus: event.registration_status ?? undefined,
    registrationFee:
      event.registration_price && event.registration_price.length > 0
        ? "유료"
        : "무료",
    registrationStartAt: toDateTimeLocal(event.registration_start_at),
    registrationEndAt: toDateTimeLocal(event.registration_end_at),
    registrationAddStartAt: toDateTimeLocal(event.registration_add_start_at),
    registrationAddEndAt: toDateTimeLocal(event.registration_add_end_at),
    registrationPrice:
      event.registration_price?.map((price) => ({
        distance: price.distance ?? "",
        price: price.price != null ? String(price.price) : "",
      })) ?? [],
    imagesCover: event.images?.cover?.length
      ? event.images.cover.map((src) => ({ src }))
      : [{ src: "" }],
    imagesDetail: event.images?.detail?.length
      ? event.images.detail.map((src) => ({ src }))
      : [{ src: "" }],
    ...
    snsInstagram: event.sns?.instagram ?? "",
    snsBlog: event.sns?.blog ?? "",
    snsYoutube: event.sns?.youtube ?? "",
  };
}

export default function DialogEventEdit({
  open,
  onOpenChange,
  event,
  onUpdated,
  ...
  const { handleSubmit, reset, control, watch } = useForm<EventAddFormValues>({
  ...
  const onSubmit = (data: EventAddFormValues) => {
    ...
    startTransition(async () => {
      try {
        const updated = await updateEventByAdmin({
          id: event.id,
  ...
  const handleDelete = () => {
    ...
    const isConfirmed = window.confirm("이 이벤트를 삭제하시겠습니까?");
    if (!isConfirmed) return;

    startTransition(async () => {
      try {
        await deleteEventByAdmin(event.id);
        onDeleted?.(event.id);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
        <DialogHeader className="border-b pb-4">
          <DialogTitle className="text-xl text-brand">이벤트 수정</DialogTitle>
          <DialogDescription>
            이벤트 기본 정보, 일정, 접수 정보를 수정할 수 있습니다.
          </DialogDescription>
        </DialogHeader>
