...
DROP TRIGGER IF EXISTS trg_events_before_update ON public.events;
CREATE TRIGGER trg_events_before_update
  BEFORE UPDATE ON public.events
  FOR EACH ROW EXECUTE FUNCTION public.fn_trg_set_updated_at();

DROP TRIGGER IF EXISTS trg_event_comments_before_update ON public.event_comments;
CREATE TRIGGER trg_event_comments_before_update
...
-- ============================================
-- 8. 이벤트 댓글 수 자동 동기화 (트리거)
-- ============================================
CREATE OR REPLACE FUNCTION public.fn_trg_event_comments_insert()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DROP TRIGGER IF EXISTS trg_event_comments_after_insert ON public.event_comments;
CREATE TRIGGER trg_event_comments_after_insert
AFTER INSERT ON public.event_comments
FOR EACH ROW EXECUTE FUNCTION public.fn_trg_event_comments_insert();

CREATE OR REPLACE FUNCTION public.fn_trg_event_comments_delete()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DROP TRIGGER IF EXISTS trg_event_comments_after_delete ON public.event_comments;
CREATE TRIGGER trg_event_comments_after_delete
AFTER DELETE ON public.event_comments
FOR EACH ROW EXECUTE FUNCTION public.fn_trg_event_comments_delete();
...
-- [트리거]
-- 1. 회원가입 시 프로필 자동 생성
-- 2. 업데이트 시 자동 갱신 (프로필, 문의, 이벤트, 댓글)
-- 3. 이벤트 댓글 수 자동 동기화 (댓글 추가/삭제)
--
-- [RPC]
-- 4. 회원 탈퇴 (Soft Delete)
-- 5. 프로필 이름 중복 확인
...
--
-- [별도 파일]
-- 9.  즐겨찾기 (supabase_event_favorites.sql) → event_favorites
-- 10. 알림설정 (supabase_event_alerts.sql)   → event_alerts
-- 11. 댓글 (supabase_event_comments.sql)     → event_comments
-- 12. 공유 기록 (supabase_event_shares.sql)  → event_shares
-- 13. 좋아요 (supabase_event_likes.sql)      → event_likes
